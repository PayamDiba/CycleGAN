...
class cycleGAN(object):
    ...
    def update_optimizer(self, input, num_nodes):
        """
        ...
        self.backwardG()
        self.average_gradients_G(num_nodes)
        ...
        self.backwardD()
        self.average_gradients_D(num_nodes)
    ...
    def sync_init(self, num_nodes):
        ...
        for param in self.dB.parameters():
            ...
            param.data = tensor1/np.sqrt(np.float(num_nodes))

        for param in self.gA.parameters():
            tensor2 = param.data
            dist.all_reduce(tensor2, op=dist.reduce_op.SUM)
            param.data = tensor2/np.sqrt(np.float(num_nodes))

        for param in self.gB.parameters():
            tensor3 = param.data
            dist.all_reduce(tensor3, op=dist.reduce_op.SUM)
            param.data = tensor3/np.sqrt(np.float(num_nodes))

    def average_gradients_D(self, num_nodes):
        """
        This function averages gradients of discriminators over all GPU nodes. On Blue Waters (I believe because of MPI backend) we need
        ...
        for param in self.dB.parameters():
            ...
            param.grad.data = tensor1.cuda()


    def average_gradients_G(self, num_nodes):
        """
        This function averages gradients of generators over all GPU nodes. On Blue Waters (I believe because of MPI backend) we need
        to transfer data back to CPU to perform comuptations and then send it back again
        to GPU.
        """
