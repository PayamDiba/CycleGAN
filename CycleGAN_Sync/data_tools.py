...
class Partition(object):
    ...
    obtained from: https://pytorch.org/tutorials/intermediate/dist_tuto.html
    """
...
class buildDataLoader_Partitioner(object):
    """
    Create multi-threaded pytorch data loaders
    """
    ...
    def getDataLoader_train(self, batch_size, transform = None, nWorkers = 1):
        """returns pytorch partitioned dataloader with the specified batch size, transformation and thread

        transformation: a torchvision transformation, if None only tensor conversion is applied
        #TODO: make sure if in cycleGAN paper any transformation such as normalization was used

        If you want to train with minimatch of size 1, specify the batch_size to be the same as number of GPUs
        """
        if not transform:
            transform = transforms.Compose([
            transforms.ToTensor(),
        ...
        bsz = batch_size / float(size) # if batch_size == #GPUS, then bsz (batch_size on each GPU) will be 1
        partition_sizes = [1.0 / size for _ in range(size)]
        partitionA = DataPartitioner(datasetA, partition_sizes)
        partitionA = partitionA.use(dist.get_rank())
        partitionB = DataPartitioner(datasetB, partition_sizes)
        partitionB = partitionB.use(dist.get_rank())


        trainLoaderA = torch.utils.data.DataLoader(partitionA, batch_size = bsz, shuffle = True, num_workers = nWorkers)
        trainLoaderB = torch.utils.data.DataLoader(partitionB, batch_size = bsz, shuffle = True, num_workers = nWorkers)

        return trainLoaderA, trainLoaderB, bsz
